...
import { Drawer, List, ListItem, ListItemText, ListItemIcon, Box, Avatar, Typography, IconButton } from '@mui/material';
...
function Navbar() {
  const [drawerOpen, setDrawerOpen] = useState(true); // Always open in large screens
...
return (
  <Box sx={{ display: 'flex' }}>
      ...
      <Box sx={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '6px', backgroundColor: '#1976d2', zIndex: 1300 }} />

      {/* 🔹 Menu Button (Visible Only on Small Screens) */}
      <IconButton
          color="inherit"
          onClick={toggleDrawer}
          sx={{ display: { xs: 'block', md: 'none' }, position: 'absolute', top: 10, left: 10, zIndex: 1400 }}
      >
          <MenuIcon />
      </IconButton>
